...
def load_img_name_list(dataset_path):
    ...
    img_name_list = [img_gt_name.split('\t')[0].strip('/') for img_gt_name in img_gt_name_list]
    return img_name_list

def load_label_name_list(dataset_path):
    img_gt_name_list = open(dataset_path).read().splitlines()
    img_name_list = [img_gt_name.split('\t')[1:] for img_gt_name in img_gt_name_list]
    return img_name_list

# TODO rename these; they aren't particularly intuitive
class ImageCaptioningDataset(CaptioningDataset):

    def __init__(self, img_name_list_path, data_root):
        super().__init__()

        self.img_name_list = load_img_name_list(img_name_list_path)[:100] # TODO remove
    ...
    def __getitem__(self, idx):
        fullpath = os.path.join(self.data_root, "images", self.img_name_list[idx])
        img = Image.open(fullpath).convert("RGB")
        return fullpath, img

class CaptioningClsDataset(ImageCaptioningDataset):

    def __init__(self, img_name_list_path, data_root):
        super(CaptioningClsDataset, self).__init__(img_name_list_path, data_root)
    ...
    def __getitem__(self, idx):
        name, img = super(CaptioningClsDataset, self).__getitem__(idx)

        mask = torch.zeros(10) # TODO decide about GT mask from pretrained model
        labels = torch.zeros(self.NUM_CLASS - 1)
        # labels from captions; we never include background
        unique_labels = torch.tensor([self.CLASS_IDX[l] for l in self.label_list[idx]],
                                     dtype=torch.long)

        assert len(unique_labels) > 0, f"No labels found for {name}"
        unique_labels -= 1 # shifting since not using background class
        labels[unique_labels] = 1
        
        return name, img, labels, mask#.astype(np.int)


class MultiscaleLoader(CaptioningClsDataset):
    ...
    def __getitem__(self, idx):
        ...
        target_size = (min(int(round(img.size[0]*scale)), self.pad_size[1]),
                       min(int(round(img.size[1]*scale)), self.pad_size[0]))
...
class COCOMultiscaleLoader(MultiscaleLoader):
    def __init__(self, img_list, cfg, transform):
        self.dataset_name = "coco"
        super().__init__(img_list, cfg, transform)

class ConceptualCaptionsMultiscaleLoader(MultiscaleLoader):
    def __init__(self, img_list, cfg, transform):
        self.dataset_name = "concap"
        super().__init__(img_list, cfg, transform)

class CropLoader(CaptioningClsDataset):
...
class COCOCropLoader(CropLoader):
    def __init__(self, img_list, cfg, transform):
        self.dataset_name = "coco"
        super().__init__(img_list, cfg, transform)

class ConceptualCaptionsCropLoader(CropLoader):
    def __init__(self, img_list, cfg, transform):
        self.dataset_name = "concap"
        super().__init__(img_list, cfg, transform)
